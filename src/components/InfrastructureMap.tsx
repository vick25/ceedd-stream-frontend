...
const createStatusIcon = (status: string) => {
  const color = status === 'good' ? 'blue' : status === 'medium' ? 'yellow' : 'red'
  return L.divIcon({
...
export function InfrastructureMap({ infrastructures, locale, height = 500 }: InfrastructureMapProps) {
            ...
            <Popup>
              <div className="text-sm min-w-[200px]">
                ...
                <div className="space-y-1">
                  <div><strong>Type:</strong> {t(`infrastructure.types.${infra.type}`, locale)}</div>
                  <div><strong>État:</strong> 
                    <span className={`ml-1 px-2 py-1 rounded text-xs text-white ${
                      infra.status === 'good' ? 'bg-blue-500' : 
                      infra.status === 'medium' ? 'bg-yellow-500' : 'bg-red-500'
                    }`}>
                      {t(`infrastructure.status.${infra.status}`, locale)}
                    </span>
                  </div>
                  <div><strong>Capacité:</strong> {getCapacityDisplay(infra)}</div>
                  <div><strong>Commune:</strong> {infra.commune}</div>
                  {infra.lastInspectionDate && (
                    <div><strong>Dernière vérification:</strong> {new Date(infra.lastInspectionDate).toLocaleDateString()}</div>
                  )}
                  {infra.fundingSource && (
                    <div><strong>Financement:</strong> {infra.fundingSource.name}</div>
                  )}
                </div>
                <div className="mt-2">
